...
def calculate_density_bins(data, kde, num_intervals):
    """根据核密度估计结果划分区间"""
    x_range = np.linspace(data.min(), data.max(), 1000)
    density = np.exp(kde.score_samples(x_range[:, None]))
    #density = np.log(density)
    cumulative_density = np.cumsum(density)  # 累计密度
...
def cluster_and_plot(csv_file, num_intervals, bandwidth):
    ...
    # 3. 根据密度划分区间
    bin_edges = calculate_density_bins(data, kde, num_intervals)

    # 4. 绘制结果
    plot_density_and_bins(data, kde, bin_edges)
    
    # 5. 输出误差率
    return clac_COS_and_NMAE(data, bin_edges)

# 替换为您的数据文件路径
csv_file = './datasets/MedBIoT/csv/100s/bilibili_100s.csv'
cluster_and_plot(csv_file, num_intervals=8, bandwidth=1)
